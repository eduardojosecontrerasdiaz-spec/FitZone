...
(function () {
    ...
    function inicializarServicios() {
        ...
        /*
         * Actualiza solamente las imágenes de los cuatro servicios
         * originales de FitZone.
         *
         * Esto evita que LocalStorage siga mostrando las imágenes
         * antiguas aunque ya hayan sido cambiadas en el proyecto.
         */
        let huboCambios = false;

        const serviciosActualizados = actuales.map(function (servicio) {
            const servicioInicial = SERVICIOS_INICIALES.find(function (inicial) {
                return inicial.id === servicio.id;
            });
            ...
            const imagenActual = servicio.foto;
            const imagenesViejas = imagenesAntiguas[servicio.id] || [];

            if (imagenesViejas.includes(imagenActual)) {
                servicio.foto = servicioInicial.foto;
                huboCambios = true;
            }
    ...
    function rutaFoto(foto, esDashboard) {
        if (!foto) {
            return (esDashboard ? "../" : "") + "assets/images/crossFit.png";
        }
        ...
        if (foto.startsWith("../") || foto.startsWith("assets/")) {
            const ruta = esDashboard
                ? (foto.startsWith("assets/") ? "../" + foto : foto)
                : foto.replace(/^\.\//, "").replace(/^\.\.\//, "");
    ...
    function renderizarInicio() {
        ...
        servicios.forEach(function (servicio) {
            ...
            tarjeta.innerHTML = `
                <img 
                    src="${rutaFoto(servicio.foto, false)}" 
                    alt="${escapar(servicio.titulo)}" 
    ...
    function renderizarDashboard() {
        ...
        if (servicios.length === 0) {
            contenedor.innerHTML =
                '<article class="tarjeta-vacio"><h3>No hay servicios registrados</h3><p>Agrega el primer servicio para mostrarlo en Inicio.</p></article>';
        ...
        servicios.forEach(function (servicio) {
            ...
            tarjeta.innerHTML = `
                <img 
                    src="${rutaFoto(servicio.foto, true)}" 
                    alt="${escapar(servicio.titulo)}" 
                    class="imagen-tarjeta"
                >
                <h3 class="titulo-tarjeta">${escapar(servicio.titulo)}</h3>
                ...
                <p class="texto-tarjeta">
                    <strong>Detalle:</strong> ${escapar(servicio.detalle)}
                </p>
                <div class="acciones-gestion">
                    <button type="button" class="boton-accion boton-editar-servicio">
                        Editar
                    </button>

                    <button type="button" class="boton-accion boton-eliminar-servicio">
                        Eliminar
                    </button>
                </div>
            `;
            ...
            tarjeta
                .querySelector(".boton-editar-servicio")
                .addEventListener("click", function () {
                    abrirFormularioServicio(servicio);
                });

            tarjeta
                .querySelector(".boton-eliminar-servicio")
                .addEventListener("click", function () {
                    if (!confirm("¿Eliminar el servicio " + servicio.titulo + "?")) {
                        return;
                    }

                    const restantes = obtenerServicios().filter(function (item) {
    ...
    function abrirFormularioServicio(servicio) {
        ...
        formulario.innerHTML = `
            <div class="formulario-entrenador-caja">
                ...
                <h2>
                    ${editar ? "Editar servicio" : "Agregar servicio"}
                </h2>
                ...
                <input 
                    id="servicioTitulo" 
                    type="text" 
                    value="${escapar(editar ? servicio.titulo : "")}" 
                ...
                <textarea 
                    ...
                    placeholder="Descripción que aparecerá en Inicio"
                >${escapar(editar ? servicio.descripcion : "")}</textarea>

                <label>Información del servicio</label>

                <textarea 
                    id="servicioDetalle" 
                    placeholder="Explica qué ofrece este servicio"
                >${escapar(editar ? servicio.detalle : "")}</textarea>

                <label>
                    Fotografía 
                    ${editar ? "(opcional para conservar la actual)" : ""}
                </label>
                ...
                <div>
                    ...
                    >
                        ${editar ? "Guardar cambios" : "Agregar servicio"}
                    </button>

                    <button 
                        id="cerrarFormularioServicio" 
                        type="button" 
                        class="boton-accion"
                    >
                        Cancelar
        ...
        document
            .getElementById("cerrarFormularioServicio")
            .addEventListener("click", function () {
                formulario.remove();
            });

        document
            .getElementById("guardarServicio")
            .addEventListener("click", function () {
                ...
                const detalle = document
                    .getElementById("servicioDetalle")
                    .value.trim();

                const archivo =
                    document.getElementById("servicioFoto").files[0];

                if (!titulo || !descripcion || !detalle) {
                    alert(
                        "Completa el nombre, la descripción y la información del servicio."
                    );
                ...
                function guardar(foto) {
                    ...
                    if (editar) {
                        const indice = servicios.findIndex(function (item) {
                            return item.id === servicio.id;
                        });

                        if (indice === -1) {
                            alert(
                                "No se encontró el servicio que intentas editar."
                            );
    ...
    document.addEventListener("DOMContentLoaded", function () {
        if (document.querySelector(".servicios-carrusel")) {
            renderizarInicio();
        }
